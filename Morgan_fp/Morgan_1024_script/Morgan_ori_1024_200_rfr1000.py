...
def rfr_prediction(folder,col):
    ...
    for i in range(200):
        x_train, x_test, y_train, y_test = train_test_split(morgan_df[file_name[i]].iloc[:, col:], morgan_df[file_name[i]]['pXC50'], test_size = 0.2)
    ...
    print(df)
    df.to_csv('Morgan_ori_1024_200_rfr1000.csv',index=False)

...
def rfr_prediction(folder):
    ...
    for obj in files:
        morgan_df[obj] = pd.read_csv(folder + "/" + obj)      
        morgan_df[obj] = morgan_df[obj][[i for i in morgan_df[obj] if morgan_df[obj][i].nunique()>1]]
    ...
    file_len = []
    names = []
    file_name = [name for name in morgan_df]

    for i in range(100):
        x_train, x_test, y_train, y_test = train_test_split(morgan_df[file_name[i]].iloc[:, 3:], morgan_df[file_name[i]]['pXC50'], test_size = 0.2)
        ...
        file_len.append(len(morgan_df[file_name[i]]))
        unique_col.append(len(morgan_df[file_name[i]].columns)-3)
    ...
    print(df)
    df.to_csv('Morgan_rad2_2048_200_rfr1000_rm_1.csv',index=False)
...
rfr_prediction('Morgan_fp_rad2_2048/')

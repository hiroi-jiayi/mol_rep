...
def rfr_prediction(folder):
    ...
    for obj in files:
        pharma_df[obj] = pd.read_csv(folder + "/" + obj)      
        pharma_df[obj] = pharma_df[obj][[i for i in pharma_df[obj] if pharma_df[obj][i].nunique()>1]]
        pharma_df[obj] = pharma_df[obj].dropna()
    ...
    file_len = []
    names = []
    file_name = [name for name in pharma_df]

    for i in range(10):
        x_train, x_test, y_train, y_test = train_test_split(pharma_df[file_name[i]].iloc[:, 3:], pharma_df[file_name[i]]['pXC50'], test_size = 0.2)
        ...
        file_len.append(len(pharma_df[file_name[i]]))
        unique_col.append(len(pharma_df[file_name[i]].columns)-3)
    ...
    print(df)
    df.to_csv('pharma3D_200_rfr1000_0_10_rm.csv',index=False)
